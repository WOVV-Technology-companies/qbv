(() => {
  ...
  const NETWORKS = [
    ...
    { id: "btc", name: "Bitcoin / BTC", coin: "BTC", path: "m/84'/0'/0'/0/{i}", kind: "bech32", hrp: "bc", sign: "btc" },
    { id: "ltc", name: "Litecoin / LTC", coin: "LTC", path: "m/84'/2'/0'/0/{i}", kind: "bech32", hrp: "ltc", sign: "btc" },
  ...
  async function deriveWallets(mnemonicInput, passphrase, accountIndex) {
    const mnemonic = validateMnemonicShape(mnemonicInput);
    const index = Number.parseInt(accountIndex, 10);
    if (!Number.isSafeInteger(index) || index < 0 || index > 1000000) throw new Error("账户序号需要是 0 到 1000000 的整数");
